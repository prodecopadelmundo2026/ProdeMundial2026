-- Keep public progress metrics on the same universe as confirmed participants:
-- authorized_emails.status = confirmed, active = true, deleted_at IS NULL.
-- Confirmed people without a profile are pending, not invisible.
...
AS $$
  ...
  knockout_alive AS (
    ...
    WHERE team IS NOT NULL
      ...
      AND team !~* '^(ganador|perdedor|winner|loser|[0-9]+\s*(Â°|Âº)?\s*(grupo|group)|[123][a-l]$)'
  )
  SELECT
    (SELECT confirmed_count FROM counts)::int AS competitors_count,
    (SELECT trial_count FROM counts)::int AS invitees_count,
    ...
    ((SELECT confirmed_count FROM counts) - (SELECT COUNT(*) FROM completed_prodes))::int AS prodes_pending_count,
    ((SELECT confirmed_count FROM counts) * 20000)::int AS prize_pool_ars,
    (SELECT COUNT(*) FROM official_results)::int AS finished_matches_count,
    CASE
